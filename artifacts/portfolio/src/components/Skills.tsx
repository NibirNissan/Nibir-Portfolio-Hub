      ...
      <div className="max-w-6xl mx-auto px-6 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {skillGroups.map((group) => (
            <div
              key={group.label}
              className={`p-8 rounded-2xl border ${group.border} bg-slate-900/60 card-hover hover:shadow-xl ${group.glow}`}
            >
              ...
              <div className="flex flex-wrap gap-2">
                {group.skills.map((skill) => (
                  <span
                    key={skill}
                    className={`skill-badge px-3 py-1.5 rounded-lg text-sm font-medium border ${group.border} ${group.bg} text-slate-300 cursor-default`}
                  >
                    {skill}
                  </span>
                ))}
